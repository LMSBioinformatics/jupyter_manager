...
from jupyter_manager import __prog__, __version__, __status__, SESSION_STORE
...
class SignalHandler(ContextDecorator):
    ...
    def _handle_interrupt(self, sig, frame) -> None:
        logger = get_logger('jupyter_signal_handler')
...
class Request:
    ...
    def __post_init__(self) -> None:
        ...
        logger = get_logger('jupyter_request')
        try:
...
class Job:
    ...
    def query(self) -> None:
        ...
        '''
        logger = get_logger('jupyter_job', self._quiet)
...
class Session(Job):
    ''' jupyter session class object, subclassing Job '''
...
def get_jupyter_jobs() -> Generator[Session, None, None]:
...
def cancel_job(job_id: str) -> None:
    ...
    Parameters:
        job_id (str): the SLURM job id
    '''
    logger = get_logger('jupyter_stop')
